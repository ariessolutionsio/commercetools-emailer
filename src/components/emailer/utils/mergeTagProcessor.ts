...
export const processMergeTags = (html: string): string => {
  ...
  processedHtml = processedHtml.replace(lineItemsRegex, (match, template) => {
    return mockData.order.orderLineItems
      .map((item) => {
        let result = template;
        result = result.replace(/{{this.productName}}/g, item.productName);
        result = result.replace(
          /{{this.productQuantity}}/g,
          String(item.productQuantity)
        );
        result = result.replace(
          /{{this.productSubTotal}}/g,
  ...
  const orderLineItemsBlockRegex =
    /<div[^>]*data-type="order-line-items"[^>]*>(.*?)<\/div>/gs;
  processedHtml = processedHtml.replace(orderLineItemsBlockRegex, (match) => {
    ...
    mockData.order.orderLineItems.forEach((item) => {
      const subtotal =
        parseFloat(item.productSubTotal.replace(/[^0-9.-]+/g, '')) *
      ...
      tableHtml += `
        <tr>
          ...
          <td style="padding: 10px; border-bottom: 1px solid #ddd;">$${subtotal.toFixed(
            2
          )}</td>
        </tr>
      `;
    ...
    tableHtml += `
          <tr>
            ...
            <td style="padding: 10px; border-top: 2px solid #ddd; font-weight: bold;">$${total.toFixed(
              2
            )}</td>
          </tr>
        </tbody>
